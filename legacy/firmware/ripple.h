...
/* b =
   [["account_set","AccountSet"],["check_cancel","CheckCancel"],["check_cash","CheckCash"],["check_create","CheckCreate"],["deposit_preauth","DepositPreauth"],["escrow_cancel","EscrowCancel"],["escrow_create","EscrowCreate"],["escrow_finish","EscrowFinish"],["offer_cancel","OfferCancel"],["offer_create","OfferCreate"],["payment","Payment"],["payment_channel_claim","PaymentChannelClaim"],["payment_channel_create","PaymentChannelCreate"],["payment_channel_fund","PaymentChannelFund"],["set_regular_key","SetRegularKey"],["signer_list_set","SignerListSet"],["trust_set","TrustSet"]];

   b.map(c=>`( msg->has_${c[0]} && !ripple_Confirm${c[1]}(node, &msg->${c[0]},
   resp) )`).join(" ||\n")

   b.map(c=>`bool ripple_Confirm${c[1]}(const HDNode *node, const Ripple${c[1]}
   *msg, RippleSignedTx *resp){}\n`).join("\n")

   b.map(c=>`bool ripple_Confirm${c[1]}(const HDNode *node, const Ripple${c[1]}
   *msg, RippleSignedTx *resp);`).join("\n")
*/
...
#define FLAG_FULLY_CANONICAL 0x80000000
#define MIN_FEE 10
#define MAX_FEE 1000000
// unsinged integer to buffer
#define ULL2B(NUM)                                                          \
  (uint8_t[]) {                                                             \
    (NUM & 0xFF00000000000000) >> 56, (NUM & 0x00FF000000000000) >> 48,     \
        (NUM & 0x0000FF0000000000) >> 40, (NUM & 0x000000FF00000000) >> 32, \
        (NUM & 0x00000000FF000000) >> 24, (NUM & 0x0000000000FF0000) >> 16, \
        (NUM & 0x000000000000FF00) >> 8, (NUM & 0x00000000000000FF) >> 0    \
  }
#define UL2B(NUM)                                       \
  (uint8_t[]) {                                         \
    (NUM & 0xFF000000) >> 24, (NUM & 0x00FF0000) >> 16, \
        (NUM & 0x0000FF00) >> 8, (NUM & 0x000000FF)     \
  }
#define US2B(NUM) \
  (uint8_t[]) { (NUM & 0xFF00) >> 8, (NUM & 0x00FF) }
#define UC2B(NUM) \
  (uint8_t[]) { (NUM & 0xFF) }
...
typedef struct TransactionField {
  ...
  const uint8_t *buf;  // if isVLEncoded is false, buf is raw data. VL will be
                       // prepended later
...
bool confirmRipplePayment(const HDNode *node, const RippleSignTx *msg,
                          RippleSignedTx *resp);
int serializeRippleTx(TransactionField_t *tf, size_t nField, bool signing,
...
void layoutRipplePayment(const char *recipient_addr, const uint64_t drops,
                         const uint32_t tag);
void layoutRippleSignerListSet(void);
